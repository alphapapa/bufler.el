...
(defun bufler-format-buffer (buffer depth)
  ...
  (let* ((modified-s (propertize (if (and (buffer-file-name buffer)
         ...
         (unique-face (bufler-tab-line-tab-face buffer nil level-face))
    ...
    (cl-labels ((format-column
      ...
      (let* ((column-sizes (nreverse column-sizes))
        ...
        (maphash (lambda (buffer column-values)
                   (let* ((face (bufler-tab-line-tab-face buffer nil nil))
                          (formatted (propertize (apply #'format format-string column-values)
                                      'face face)))
                     (puthash buffer formatted table)))
                 table)
