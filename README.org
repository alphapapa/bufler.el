...
6.  Maybe make a new frame for a different project: use =bufler-list= to show the list of buffer groups, then select a group and press =f= to make the frame.  Then in that frame, use =C-u M-x bufler-switch-buffer= or =M-x bufler-frame-workspace= to set the frame to a different workspace.
