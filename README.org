...
+  =bufler-set-workspace= :: Set the frame's workspace.  Setting the workspace is usually handled by =bufler-switch-buffer=, but this command may be used to set the workspace to a group containing other groups.
