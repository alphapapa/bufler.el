...
To show Bufler's grouped buffers in a Helm-specific command, Bufler provides =helm-bufler.el=, which includes =helm-bufler-source=, a Helm source that shows buffers in the current workspace (or when the Helm command is called with =C-u=, all buffers).  It looks like this when showing all buffers:
