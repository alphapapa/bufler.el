...
This screenshot compares Bufler and Ibuffer showing the same buffers with the default settings.  Note how Bufler allows commands to be applied to selected sections: for example, pressing =k= in the Bufler list would kill the buffers in the groups =compilation-mode= and =completion-list-mode= (and, of course, since it calls =kill-buffer=, any unsaved, file-backed buffers would ask to be saved first).
